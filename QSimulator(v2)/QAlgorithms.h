...
namespace Work_namespace {
	...
	long Grover_Search(const Matrix<std::complex<double>>& Oracle, long size,
		long& final_iterations_col, long roots) {
		long initial_space_size = 1 << size;
		long space_size = Oracle.NumCols();
		...
		Matrix<std::complex<double>> R_transform;
		R_transform.SetVals(R);
		long counter = 0;
		do {
